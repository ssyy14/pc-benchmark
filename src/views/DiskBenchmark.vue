...
import { calculateDiskScore } from '../utils/scoring'
...
const result = computed(() => benchmark.resultMap['disk'])

function handleProgress(p: BenchmarkProgress) {
  if (p.type === 'disk') progress.value = p
}

async function start() {
  isRunning.value = true
  benchmark.isRunning = true
  benchmark.runningType = 'disk'
  window.benchmarkAPI.onProgress(handleProgress)
  try {
    const r = await window.benchmarkAPI.runBenchmark('disk')
    const score = calculateDiskScore(r.metrics)
    benchmark.setResult({ ...r, score })
  } catch (e) {
    console.error('Disk benchmark failed:', e)
  } finally {
...
<template>
  <div>
    <div class="flex items-center justify-between mb-6">
      <div>
        <h2 class="page-title">💾 {{ $t('nav.disk') }}</h2>
        <p class="page-subtitle">Sequential and random 4K read/write</p>
      </div>
    ...
    <div v-if="isRunning && progress" class="bench-card mb-6">
      ...
      <div v-if="progress.metrics" class="grid grid-cols-2 gap-4 mt-4 text-center">
        <div v-if="progress.metrics.seqReadMBps">
          <div class="text-2xl font-bold text-blue-600">{{ progress.metrics.seqReadMBps }}</div>
          <div class="text-xs text-gray-400">Seq Read MB/s</div>
        </div>
        <div v-if="progress.metrics.seqWriteMBps">
          <div class="text-2xl font-bold text-blue-600">{{ progress.metrics.seqWriteMBps }}</div>
          <div class="text-xs text-gray-400">Seq Write MB/s</div>
        </div>
      </div>
    </div>
