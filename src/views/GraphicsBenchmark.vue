...
function handleProgress(p: BenchmarkProgress) {
  if (p.type === 'graphics') progress.value = p
}
...
async function start() {
  ...
  try {
    const r = await window.benchmarkAPI.runBenchmark('graphics')
    const score = calculateGraphicsScore(r.metrics)
    benchmark.setResult({ ...r, score })
  } catch (e) {
    console.error('Graphics benchmark failed:', e)
  } finally {
  ...
  <div>
    <div class="flex items-center justify-between mb-6">
      <div>
        ...
        <p class="page-subtitle">Particle simulation and 2D rendering at multiple resolutions</p>
      </div>
    ...
    <div v-if="isRunning && progress" class="bench-card mb-6">
      ...
      <div v-if="progress.metrics" class="grid grid-cols-3 gap-4 mt-4 text-center">
        <div v-if="progress.metrics.fps1080p">
          <div class="text-2xl font-bold text-blue-600">{{ progress.metrics.fps1080p }}</div>
          <div class="text-xs text-gray-400">1080p FPS</div>
        </div>
        <div v-if="progress.metrics.fps720p">
          <div class="text-2xl font-bold text-blue-600">{{ progress.metrics.fps720p }}</div>
          <div class="text-xs text-gray-400">720p FPS</div>
        </div>
        <div v-if="progress.metrics.fps480p">
          <div class="text-2xl font-bold text-blue-600">{{ progress.metrics.fps480p }}</div>
          <div class="text-xs text-gray-400">480p FPS</div>
        </div>
      </div>
    </div>
